...
SELECT DATE_PART('year', ood.order_purchase_timestamp) AS year,
	...
	SUM(ood.order_delivered_carrier_date - ood.order_purchase_timestamp) / COUNT(DISTINCT ood.order_id) AS avg_purchase_to_carrier,
	SUM(ood.order_delivered_customer_date - ood.order_delivered_carrier_date) / COUNT(DISTINCT ood.order_id) AS avg_carrier_to_customer,
	ROUND(SUM(oord.review_score)::NUMERIC / COUNT(ood.order_id), 2) AS avg_ratings
